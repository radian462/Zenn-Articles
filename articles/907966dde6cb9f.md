...
以前は、勝手に止められることがありましたが、現在は停止条件が明文化されて使いやすくなりました。
この手のサービスにしては珍しく、日本語に対応しています。
日本リージョンはないものの、インドネシアとアメリカリージョンが使えます。
Quotaを超過すると、プロジェクトが停止され、**1週間以内にプランをアップグレードしないとプロジェクトごと削除される**のは注意。
軽く放置してみた感じ、1日で$0.03くらい消費するとわかりました。余程の高負荷の処理を常時走らせない限り、Discord Botなら$5以内に収まりそうです。

| [**無料枠**](https://zeabur.com/ja-JP/pricing) |  | 
| ---- | ---- |
| CPU | $0/vCPU (1分あたり)|
| メモリ | $0.00025/GB (1分あたり)|
| 転送 | $0.1/GB |
| ストレージ | $0.2/GB |
| Quota | $5/月 |
| カード登録 | 不要 |
...
2025/12/16 カード情報を追加、Zeaburは起動確認済みに再分類